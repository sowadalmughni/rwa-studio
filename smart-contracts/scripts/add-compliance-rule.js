...
async function addComplianceRule(taskArgs, hre) {
  ...
  try {
    ...
    switch (ruleType) {
      case "investor-limit": {
        const maxInvestors = params[0] ? parseInt(params[0]) : ruleConfig.defaultParams.maxInvestors;
      ...
      case "geographic": {
        ...
        ruleContract = await GeographicRule.deploy(tokenAddress, identityRegistryAddress, isAllowlistMode);
        break;
      }
      ...
      case "accredited": {
        ...
        const AccreditedInvestorRule = await hre.ethers.getContractFactory("AccreditedInvestorRule");
        ruleContract = await AccreditedInvestorRule.deploy(tokenAddress, identityRegistryAddress);
        break;
      }
      
      case "holding-period": {
        const holdingPeriodDays = params[0] ? parseInt(params[0]) : ruleConfig.defaultParams.holdingPeriodDays;
        const holdingPeriodSeconds = holdingPeriodDays * 24 * 60 * 60;
        console.log(`   Holding Period: ${holdingPeriodDays} days (${holdingPeriodSeconds} seconds)`);
