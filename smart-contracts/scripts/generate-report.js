...
async function generateComplianceReport(taskArgs, hre) {
  ...
  try {
    ...
    if (identityRegistryAddress !== hre.ethers.ZeroAddress) {
      ...
      try {
        identityInfo.verifiedAddressCount = Number(await identityRegistry.getVerifiedAddressCount());
      } catch (e) {
    ...
    switch (format) {
      ...
      case "pdf":
        ...
        console.log(`\n⚠️ Note: PDF output generates Markdown. Use a tool like pandoc to convert to PDF.`);
        break;
...
function generateMarkdown(report) {
  ...
  if (report.compliance.rules.length > 0) {
    ...
    lines.push("|------|-------------|--------|");
    for (const rule of report.compliance.rules) {
      lines.push(`| \`${rule.address.slice(0, 10)}...\` | ${rule.description} | ${rule.isActive} |`);
    }
  } else {
